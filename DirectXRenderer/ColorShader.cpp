...
namespace Ab2
{
	...
	bool ColorShader::Initialize(ID3D11Device* a_pDevice, SceneSettings* a_pSceneSettings)
	{
		bool result;

		result = InitializeShader(a_pDevice, "../colorvs.cso", "../colorps.cso");
		if(!result)
		{
			return false;
		}
	...
	bool ColorShader::Render(ID3D11DeviceContext* a_pDeviceContext, int a_nIndexCount, XMFLOAT4X4 a_mWorldMatrix, XMFLOAT4X4 a_mViewMatrix, XMFLOAT4X4 a_mProjectionMatrix)
	{
		bool result;
	...
	bool ColorShader::InitializeInputLayout(ID3D11Device* a_pDevice, char* a_pVertexCsoBuffer, size_t a_nVertexCsoSize)
	{
		HRESULT result;
	...
	bool ColorShader::SetShaderParameters(ID3D11DeviceContext* a_pDeviceContext, XMFLOAT4X4 a_mWorldMatrix, XMFLOAT4X4 a_mViewMatrix, XMFLOAT4X4 a_mProjectionMatrix)
	{
		HRESULT result;
